...
# Triggering Flask auto-reload again 3
@app.route('/')
def index():
    if 'user' in session:
...
def dashboard():
    ...
    user_high_risk_urls = sum(1 for item in user_history if item.input_type == 'URL' and item.risk_level in ['PHISHING', 'Invalid'])
    user_high_risk_sms = sum(1 for item in user_history if item.input_type == 'SMS' and item.risk_level in ['SMISHING', 'PHISHING', 'Invalid'])
    user_safe_scans = sum(1 for item in user_history if item.risk_level in ['SAFE'])
...
def admin_dashboard():
    ...
    # Exclude only empty text
    history = [h for h in history if h.input_text.strip() != '']
    user_data = []
    for u in users:
...
def api_telemetry():
    ...
    # Distributions
    risk_distribution = {
        'High': sum(1 for h in history if h.risk_level in ['PHISHING', 'SMISHING', 'Invalid']),
        'Medium': sum(1 for h in history if h.risk_level == 'SUSPICIOUS'), # Leftover if any old logs
        'Low/Safe': sum(1 for h in history if h.risk_level in ['SAFE'])
    }
    type_distribution = {
...
def api_admin_telemetry():
    ...
    # Calculate Stats
    total_scans = len(history)
    phishing_urls = sum(1 for h in history if h.input_type == 'URL' and h.risk_level in ['PHISHING', 'Invalid'])
    suspicious_sms = sum(1 for h in history if h.input_type == 'SMS' and h.risk_level in ['SMISHING', 'PHISHING', 'Invalid', 'Medium', 'SUSPICIOUS'])
    safe_messages = sum(1 for h in history if h.risk_level in ['SAFE', 'Low', 'Safe'])
    high_risk_alerts = sum(1 for h in history if h.risk_level in ['PHISHING', 'SMISHING', 'Invalid'])
    
    # Distributions
    risk_distribution = {
        'High': high_risk_alerts,
        'Medium': sum(1 for h in history if h.risk_level in ['Medium', 'SUSPICIOUS']),
...
def threat_analytics():
    ...
    user_phishing = sum(1 for item in user_history if item.risk_level in ['PHISHING', 'SMISHING', 'Invalid'])
    user_safe = sum(1 for item in user_history if item.risk_level in ['SAFE'])
...
if __name__ == '__main__':
    with app.app_context():
        ...
        # Database is retained across reloads.
        pass
