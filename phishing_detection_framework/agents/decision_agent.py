...
class DecisionAgent:
    ...
    def analyze_input(self, text, input_type="URL", high_confidence=False):
        ...
        if input_type.upper() == "URL":
            ...
            # Keyword heuristic for obvious fake/smishing SMS
            smishing_keywords = [
                'urgent', 'bank', 'account', 'blocked', 'verify', 'password', 
                'click', 'prize', 'winner', 'login', 'suspended', 'alert', 
                'fake', 'update', 'won', 'free', 'claim', 'card', 'details', 
                'reward', 'sensitive', 'gift', 'cash', 'lottery', 'offer'
            ]
            if any(kw in text_stripped.lower() for kw in smishing_keywords):
                result = {
                    "type": "SMS",
                    "input": text,
                    "prediction": "Smishing",
    ...
    def _format_decision(self, result, high_confidence):
        ...
        if result["type"] == "URL":
            ...
            if is_safe_brand or risk_level == "SAFE":
                ...
                domain_intel["Domain Age"] = "< 1 Year (Suspicious)"
                domain_intel["IP Location"] = "High Risk Region"
                domain_intel["WHOIS Status"] = "Privacy Protected"

        if risk_level in ["PHISHING", "SMISHING"]:
            if result["type"] == "URL":
            ...
            if is_safe_brand:
                ...
                indicators = ["No known malicious patterns detected", "Standard vocabulary and formatting", "Risk score is strictly below 40"]
